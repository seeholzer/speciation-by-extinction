##################################################
#	Description: Create Figure 2 and Figure S1
...
for(v in c('v1','v2','v3')){
	...
	key = data.frame(scenario=c('FULL','SBE3','SBE2','SBE1'),
						...
						x=-81.25,
						y=c(-10,-11.5,-11,-10.5)+.75)
	
	bg = mapvalues(popdata$scenario,key[,1],key[,2])
	text.col = mapvalues(popdata$scenario,key[,1],key[,'text.col'])
	...
	text(key$x,key$y,key[,1],cex=1,adj=c(0,.5))
	offset = 1.35
	
	x = key[key$scenario %in% c('FULL'),]
	text(x$x+offset,x$y,expression('\U2012'),c(.5,1))
	
	x = key[key$scenario %in% c('SBE1'),]
	points(unique(x$x)+seq(0,0,0.5)+offset,rep(x[x$scenario %in% 'SBE1','y'],1),pch=21,bg=rev(x$col),col='grey50',cex=2)
	
	x = key[key$scenario %in% c('SBE1','SBE2'),]
	points(unique(x$x)+seq(0,0.5,0.5)+offset,rep(x[x$scenario %in% 'SBE2','y'],2),pch=21,bg=rev(x$col),col='grey50',cex=2)
	
	x = key[key$scenario %in% c('SBE1','SBE2','SBE3'),]
	points(unique(x$x)+seq(0,1,0.5)+offset,rep(x[x$scenario %in% 'SBE3','y'],3),pch=21,bg=rev(x$col),col='grey50',cex=2)
	
	x = key[key$scenario %in% c('FULL'),]
	text(x$x+offset,x$y+.6,expression(italic('in silico')),adj=c(.25,.5),cex=.75)
	text(x$x+offset,x$y+.35,'extinction',adj=c(.25,.5),cex=.75)
