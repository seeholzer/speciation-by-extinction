##################################################
#	Description: sample assignment maps for mclust analysis of C. antisiensis
#		- produces maps of mclust cluster assignments to each population for all scenarios
#
##################################################
...
colkey = data.frame(class=1:2,col=c('#d7191c','#2c7bb6'),stringsAsFactors=F)


dir = 'mclust.assignment.maps'
if(!dir.exists(dir)){
		dir.create(dir)
...
for(i in which(sapply(results.list,function(x)x$trait.set) == 'combined')){
	...
	png(paste0('mclust.assignment.maps/',trait.set,'.', scenario.name,'_map.png'),		width=5*w,height=5*h,units='in',res=300,bg='transparent')
	...
	if(grepl('FULL',scenario.name)){
		text(-81.1,-11,'Pre-Extinction',cex=1.25)
	}
	
	if(grepl('EXT',scenario.name)){
		text(-79.5,-11,'Post-Extinction',cex=1.25)
		text(-79.5,-11.6,paste0('Scenario ',gsub('EXT_v','',scenario.name)),cex=1.25)	
	}
